...
protected:
	BOOL InitHooks(DWORD dwOptions = HM_CALLWNDPROC, LPCTSTR szClassFilter = NULL)
	{
	  ...
	  if(szClassFilter!=NULL)m_sClassFilter = szClassFilter;
...
#ifdef _UNICODE
	std::wstring  m_sClassFilter;
#else
	std::string   m_sClassFilter;
#endif   
	BOOL m_b2000orLater;
...
protected:
	...
	virtual BOOL OnMouseEx(UINT /*uMouseMsg*/, const _MOUSEHOOKSTRUCTEX& /*info*/) { ASSERT (0); return FALSE; }

	inline BOOL ClassMatches(HWND hwnd)
	{
		return ::ClassMatches(hwnd, m_sClassFilter.c_str());
	}
      ...
      MGRTYPE& mgr = GetInstance();
		...
		if (nCode == HC_ACTION)
		{
			...
			if (mgr.ClassMatches(pInfo->hwnd))
         {
            ...
            {
               _MOUSEHOOKSTRUCTEX* pInfoEx = (_MOUSEHOOKSTRUCTEX*)pInfo;
