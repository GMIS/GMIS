...
bool CRequestBrain::Request(CTaskDialog* Dialog){
...
bool CNameList::HasName(CTaskDialog* Dialog,tstring& Name){
...
CTaskDialog::CTaskDialog(CBrain* Brain,int64 SourceID,int64 DialogID,int64 ParentDialogID,tstring SourceName,tstring DialogName,
						 DIALOG_TYPE Type,int64 OutputSourceID,TASK_OUT_TYPE TaskType)
:m_TaskMsgList(&m_Mutex)
{
	...
	Reset(Brain,SourceID,DialogID,ParentDialogID,SourceName,DialogName,Type,OutputSourceID,TaskType);
}
...
void CTaskDialog::Reset(CBrain* Brain,int64 SourceID,int64 DialogID,int64 ParentDialogID,tstring SourceName,tstring DialogName,
		   DIALOG_TYPE Type,int64 OutputSourceID,TASK_OUT_TYPE TaskType)

{
		...
		m_bEditValid = true;
		m_EditText = _T("");
		m_TaskStateLastTimeStamp = 0;
		m_OutputSourceID = OutputSourceID;
...
Energy*  CTaskDialog::ToEnergy(){
	...
	if (Pipe)
	{
		...
		if (m_TaskID!=0)
		{
			CBrainTask* Task  = m_Brain->GetBrainData()->GetLogicTask(m_TaskID,false); 
...
bool   CTaskDialog::FromEnergy(Energy* e){
	...
	{
		Pipe->Pop(&E);
		CBrainTask* Task = m_Brain->GetBrainData()->CreateLogicTask(GetDialogID(),m_TaskID);
		if(!Task){
			return false;
		}
	...
	{
		Pipe->Pop(&E);
		CLogicThread* Think = m_Brain->GetBrainData()->CreateLogicThread(GetDialogID(),m_ThinkID);
		if(!Think){
			return false;
		}
...
void CTaskDialog::NotifyPause(ePipeline& ExePipe,ePipeline& Address){  
	...
	CRequestBrain rq(REQUEST_DIALOG,EventID);
...
void CTaskDialog::ClosePauseDialog(int64 PauseEventID)
{
	map<int64,int64>::iterator it = m_PauseEventList.begin();
	while (it != m_PauseEventList.end())
	{
		...
		{		
			CRequestBrain rq(REQUEST_END,it->second);
			rq.Request(this);
...
int64 CTaskDialog::GetFocusPauseItemID(){
...
void CTaskDialog::GetPauseIDList(ePipeline& List){
	map<int64,int64>::iterator it = m_PauseEventList.begin();
	while(it != m_PauseEventList.end()){
...
void CTaskDialog::StopPause(int64 PauseID,int64 CmdID){
	if (PauseID ==0)
	{
		ePipeline PauseEventList;
		map<int64,int64>::iterator it = m_PauseEventList.begin();
        ...
        while(PauseEventList.Size())
		{
			...
			bool ret = m_Brain->GetBrainData()->PopBrainEvent(EventID,ObjectInfo);
			if (!ret)
			{
				return;
			}
			
			ePipeline* ObjectAddress1 = (ePipeline*)ObjectInfo.GetData(EVENT_ADDRESS);
			ePipeline* ExePipe1 = (ePipeline*)ObjectInfo.GetData(EVENT_PIPE);
			
			CMsg EltMsg(*ObjectAddress1,MSG_ELT_TASK_CTRL,EventID);
			ePipeline& Letter = EltMsg.GetLetter();
			Letter.PushInt(CmdID);
			Letter.PushPipe(*ExePipe1);
			
			//给Element发信息
			m_Brain->PushNerveMsg(EltMsg);
...
bool CTaskDialog::StartChildDialog(int64 EventID,tstring DialogName,tstring FirstDialog,TASK_OUT_TYPE OutType,ePipeline& ExePipe,ePipeline& Address,int64 EventInterval/*=TIME_SEC*/ ,bool bFocus /*=false*/,bool bEditValid /*=true*/){
...
void CTaskDialog::CloseChildDialog(int64 EventID,ePipeline& OldExePipe,ePipeline& ExePipe){

	assert(EventID !=0);
	
    CRequestBrain rq(REQUEST_END,EventID);
    rq.Request(this);
...
ePipeline&  CTaskDialog::SaveReceiveItem(tstring Info,int32 State){
...
ePipeline&  CTaskDialog::SaveSendItem(tstring Info, int32 State)
{
...
void  CTaskDialog::SendMsg(CMsg& Msg,POS_TYPE Type){
	
	if (Type == LOCAL)
	{
		if (m_SourceID == SYSTEM_SOURCE)
		{
			m_Brain->PushCentralNerveMsg(Msg);
		}else{
			if (GetTaskState()==TASK_DELELTE)
			{
				return;
			}
			m_Brain->PushNerveMsg(Msg);
		}
	} 
	else
	{
		...
		if (m_SourceID == SYSTEM_SOURCE)
		{
			...
			CLinker Linker;
			m_Brain->GetLinker(m_SourceID,Linker);
			if (Linker.IsValid())
			{
...
void CTaskDialog::ClearTaskMsgList()
{
	while (m_TaskMsgList.DataNum())//关于此任务的信息直接丢弃
	{
		assert(0); //测试
		CMsg m;
		m_TaskMsgList.Pop(m);
...
void CTaskDialog::FeedbackToBrain(){

	int64 EventID = GetDialogID();
    
	if(EventID != 0){
		CMsg rMsg(MSG_TASK_FEEDBACK,EventID,EventID);		
		ePipeline& rLetter = rMsg.GetLetter();
		rLetter.PushPipe(m_ExePipe);	
		
		m_Brain->PushCentralNerveMsg(rMsg);	
	}else{
...
void  CTaskDialog::Do(CMsg& Msg){
	...
	if (TaskAddress.Size()==0)  //系统对话
	{
		...
		if (m_DialogID == DEFAULT_DIALOG)
		{
			m_Brain->BrainDialogProc(this,Msg);	
		}else{
			m_Brain->TaskDialogProc(this,&m_ExePipe,Msg);
		}
	}
    else{
...
void CTaskDialog::SetBreakPoint(ePipeline& Path,BOOL bEnable){
	CBrainTask* CurTask  = m_Brain->GetBrainData()->GetLogicTask(m_TaskID);
    assert(CurTask);
...
TASK_STATE   CTaskDialog::SetTaskState(TASK_STATE State){
	CLock((CABMutex*)&m_Mutex);
	TASK_STATE Old = m_TaskState;
	m_TaskState = State;
	m_TaskStateLastTimeStamp = AbstractSpace::CreateTimeStamp();

	return Old;
}
TASK_STATE  CTaskDialog::GetTaskState(){
	CLock((CABMutex*)&m_Mutex);
	TASK_STATE State = m_TaskState ;
    return State;
}

TASK_OUT_TYPE CTaskDialog::GetTaskType(){
	return m_TaskOutType;
}
void CTaskDialog::NotifyTaskState()
{
	TASK_STATE State = m_TaskState;
...
bool CTaskDialog::ResumeTask(){
	...
	while(it != m_PauseEventList.end())
	{
		...
		if (!NewDialog)
		{
			NewDialog = m_Brain->GetBrainData()->CreateNewDialog(this,m_SourceID,EventID,GetDialogID(),_T("Dialog"),m_DialogName,DIALOG_SYSTEM_CHILD,0,OutType);	
...
int32 CTaskDialog::GetSysProcNum(){
	CLock((CABMutex*)&m_Mutex);
	int32 n = m_SysProcCounter;
	return n;
}
void  CTaskDialog::ResetThink(){
	m_ThinkID = 0;
	m_bEditValid = TRUE;
	m_EditText = _T("");
}
void  CTaskDialog::ResetTask(){
	m_TaskID = 0;
	m_ExePipe.Reuse();
	m_ExePipe.Clear();
	...
	{
		m_Brain->GetBrainData()->DeleteDialog(m_ControlDialogID);
    }
...
void CTaskDialog::SetWorkMode(WORK_MODE Mode){
	CLock lk(&m_Mutex);
    m_WorkMode = Mode;
}

WORK_MODE CTaskDialog::GetWorkMode(){
	CLock lk(&m_Mutex);
	WORK_MODE Mode = m_WorkMode;
	return Mode;
}

CLogicThread* CTaskDialog::GetThink(){
	if(m_ThinkID ==0){
		CLogicThread* Think = m_Brain->GetBrainData()->CreateLogicThread(GetDialogID());
...
CBrainTask* CTaskDialog::GetTask(){
	if(m_TaskID==0){
		CBrainTask* Task = m_Brain->GetBrainData()->CreateLogicTask(GetDialogID());
		m_TaskID = Task->m_ID;
		return Task;
	};
	CBrainTask* Task = m_Brain->GetBrainData()->GetLogicTask(m_TaskID);
	return Task;
}

CTaskDialog& CTaskDialog::operator << (CTaskDialog& Dialog){
	...
	m_Brain->GetBrainData()->SetLogicThreadUser(m_ThinkID,GetDialogID());
	m_Brain->GetBrainData()->SetLogicTaskUser(m_TaskID,GetDialogID());
...
void CTaskDialog::RuntimeOutput(tstring s){
...
void CTaskDialog::RuntimeOutput(int64 MassID,tstring s){
...
void CTaskDialog::RuntimeOutput(int64 MassID,TCHAR* Format, ...){
	TCHAR Buffer [512] ;
	va_list ArgList ;		
...
bool CTaskDialog::CompileTask(){
	...
	if(m_SpaceType==DIALOG_OTHER_CHILD){  //是输入给对方，让对方执行
...
void CTaskDialog::SentenceCompileProgress(int32 ClauseNum, int32 Completed){
