...
CLocalLogicCell* CTaskDialog::FindLogic(const tstring& Name){
...
CElementCell* CTaskDialog::FindCapacitor(const tstring& Name){
...
CElementCell* CTaskDialog::FindInductor(const tstring& Name){
...
CObjectData* CTaskDialog::FindObject(int64 ObjectID){
    //优先在本地找
	...
	if (m_ParentDialogID)
	{
		CTaskDialog* ParentDialog = m_Brain->GetBrainData()->GetDialog(m_ParentDialogID);
		if (ParentDialog)
		{
    ...
    //最后在系统对话中找
	CTaskDialog* SysDialog = m_Brain->GetBrainData()->GetDialog(DEFAULT_DIALOG);
	if (SysDialog)
	{
...
int32 CTaskDialog::FindObject(tstring Name,vector<CObjectData>& ObjectList){
...
bool CTaskDialog::RegisterLogic(CBrainTask* Task)
{	
...
void CTaskDialog::DeleteLogic(const tstring& Name){
	...
	CBrainTask* Task = &lg->m_Task; //因为数据已经转给了此TASK
...
void CTaskDialog::ReferenceLogic(const tstring& scrName,const tstring& refName,CLocalLogicCell* WhoRef,bool Add){
...
void CTaskDialog::RegisterCapacitor(const tstring& Name, CLocalLogicCell* CreatedBy){
...
void CTaskDialog::ReferenceCapacitor(const tstring& scrName,const tstring& refName, CLocalLogicCell* WhoRef,bool Add)
{
...
void CTaskDialog::RegisterInductor(const tstring& Name, CLocalLogicCell* CreatedBy){
...
void CTaskDialog::RegisterObject(ePipeline& ObjectData){
...
void CTaskDialog::DeleteCapacitor(const tstring& Name)
{
	vector<CElementCell*>::iterator It = m_CapaList.begin();
	while(It<m_CapaList.end()){
		if((*It)->m_Name == Name){
...
void CTaskDialog::DeleteInductor(const tstring& Name)
{
	vector<CElementCell*>::iterator It = m_InduList.begin();
	while(It<m_InduList.end()){
		if((*It)->m_Name == Name){
...
void CTaskDialog::DeleteObject(ePipeline& ObjectData){
...
void CTaskDialog::ReferenceInductor(const tstring& scrName,const tstring& refName, CLocalLogicCell* WhoRef,bool Add)
{
...
ePipeline*  CTaskDialog::FindTempMemory(int64 InstanceID){
...
CElement*  CTaskDialog::FindLogicInstance(int64 InstanceID){
...
ePipeline* CTaskDialog::FindObjectInstance(int64 InstanceID){
...
void CTaskDialog::AddObjectInstance(int64 InstanceID,ePipeline& Pipe){
	m_ObjectInstanceList[InstanceID] = Pipe;

}
void CTaskDialog::AddMemoryInstance(int64 InstanceID, ePipeline& Pipe){
	m_TableInstanceList[InstanceID] = Pipe;
}

void CTaskDialog::CloseObjectInstance(int64 InstanceID){
	map<int64,ePipeline>::iterator it = m_ObjectInstanceList.find(InstanceID);
	if (it != m_ObjectInstanceList.end())
	{
...
void CTaskDialog::CloseMemoryInstance(int64 InstanceID){
	map<int64,ePipeline>::iterator it = m_TableInstanceList.find(InstanceID);
	if (it != m_TableInstanceList.end())
	{
...
void CTaskDialog::ClearLogicSence(){
    vector<CLocalLogicCell*>::iterator It = m_LogicList.begin();
	while(It!=m_LogicList.end()){
		delete *It;
		*It = NULL;
...
void CTaskDialog::GetLocalLogicData(ePipeline& List){
	ePipeline  Pipe;
	vector<CLocalLogicCell*>::iterator It = m_LogicList.begin();
	while(It!=m_LogicList.end()){
...
void CTaskDialog::GetLocalCapaData(ePipeline& List){
	ePipeline Pipe;
    vector<CElementCell*>::iterator Ita = m_CapaList.begin();
	while(Ita!=m_CapaList.end()){
...
void CTaskDialog::GetLocalInduData(ePipeline& List){
	ePipeline Pipe;
...
void CTaskDialog::GetLocalObjectData(ePipeline& List){
...
void CTaskDialog::GetTableInstanceData(ePipeline& List){
