...
CBrainTask::CBrainTask(CBrainTask* Parent)
:CSeries(0,_T("Logic")){
	Reset(0,-1);
	m_Parent = Parent;
}

CBrainTask::CBrainTask(int64 TaskID,int64 UserDialogID)
:CSeries(0,_T("Task"))
{
...
void CBrainTask::Reset(int64 TaskID,int64 UserDialogID){
...
CBrainTask* CBrainTask::GetRootTask(){
	if (m_Parent==NULL)
	{
		return this;
	}

	CBrainTask* Task = (CBrainTask*)m_Parent;
	return Task->GetRootTask();
}

Energy*  CBrainTask::ToEnergy(){
...
bool     CBrainTask::FromEnergy(CTaskDialog* Dialog,Energy* E){
...
CBrainTask& CBrainTask::operator<<(CBrainTask& Task){
	if (&Task == this)
	{
		return *this;
	}
...
bool  CBrainTask::SetBreakPoint(ePipeline& Path,BOOL bEnable ){
...
void  CBrainTask::GetDebugItem(ePipeline& ParentPipe,Mass* m){
...
bool CBrainTask::Compile(CTaskDialog* Dialog,ePipeline* Sentence){
...
CElement* CBrainTask::CompileSentence(CTaskDialog* Dialog,tstring& LogicText,ePipeline* Sentence)
{	
		...
		if(BelongInstinct(InstinctID)){
			...
			else if(InstinctID == INSTINCT_USE_LOGIC)
			{
				...
				if (LogicName[0] == _T('<')) //全局
				{
					...
					CBrainTask* RootTask = GetRootTask();

					CBrainTask NestTask(RootTask);
...
Mass* CBrainTask::GetInstinctInstance(CTaskDialog* Dialog,int64 InstinctID, Energy* Param){
