...
void CBrain::TaskDialogProc(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBroadcaseMsg(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
	//所谓Server暂时没有处理广播信息的必要
	assert(0);
}


void CBrain::OnCloseDialog(CTaskDialog*Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnMsgFromBrain(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg)
{
...
void CBrain::OnBrainTextInputing(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){

	Dialog->GetThink()->ThinkProc(Dialog,Msg);
};

void CBrain::OnBrainTextInputEnd(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
	ePipeline& Letter = Msg.GetLetter();
...
void CBrain::OnBrainGetMoreLog(CTaskDialog*Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBrainSetFocusDialog(CTaskDialog*Dialog,ePipeline* ExePipe,CMsg& Msg){
	ePipeline& Letter  = Msg.GetLetter();
...
void CBrain::OnBrainTaskControl(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
	...
	case CMD_EXE:
		{
			...
			PushNerveMsg(TaskMsg); //由于可能改变了任务内部，导致迭代器失效，所以必须经由系统分发信息
		}
		break;
    case CMD_STOP:
		{
...
void CBrain::OnBrainGetDebugItem(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
	ePipeline& Letter = Msg.GetLetter();
	...
	{
		CBrainTask* Task = GetBrainData()->GetLogicTask(Dialog->m_TaskID);
...
void CBrain::OnBrainLogicOperate(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
	ePipeline& Letter = Msg.GetLetter();
...
void CBrain::OnBrainObjectOperate(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg)
{
	ePipeline& Letter = Msg.GetLetter();
...
void CBrain::OnBrainMemoryOperate(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg)
{
...
void CBrain::OnBrainGetThinkResult(CTaskDialog* Dialog, ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBrainGetAnalyseResult(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBrainClearDialogOutput(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
	Dialog->m_RuntimeOutput.Clear();
	//此处应该通知界面的，暂时简化如此
}

void CBrain::OnBrainGetFindResult(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBrainClearThink(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBrainConnectTo(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg){
...
void CBrain::OnBrainDisconnect(CTaskDialog*Dialog,ePipeline* ExePipe,CMsg& Msg){	
	CLinker Linker;
...
void CBrain::OnBrainSetLogFlag(CTaskDialog* Dialog,ePipeline* ExePipe,CMsg& Msg)
{
	ePipeline& Letter = Msg.GetLetter();
...
void CBrain::OnTaskResult(CTaskDialog*Dialog,ePipeline* ExePipe,CMsg& Msg){	
	ePipeline& Letter = Msg.GetLetter();
...
void CBrain::OnEventTick(CTaskDialog*Dialog,ePipeline* ExePipe,CMsg& Msg){
	int64 EventID = Msg.GetEventID();
	ePipeline& Letter = Msg.GetLetter();
