...
void CBrain::CLockedBrainData::RegisterEvent(int64 EventID,int64 ClientEventID,ePipeline& ExePipe,ePipeline& LocalAddress,int64 EventInterval, bool bEventOnce,int64 InstinctID,bool bPrivate){
	...
	Event.m_InstinctID = InstinctID;
	Event.m_bPrivate = bPrivate;

	assert(Event.m_ClientAddress.Size());
	//事件总是按时间先后顺序生成的
	assert(m_EventList.rbegin()->first == EventID);
};

bool CBrain::CLockedBrainData::GetEvent(int64 EventID,CBrainEvent& EventInfo,bool bDelete){
	_CLOCK(&m_EventMutex);
	...
	while (Brain->IsAlive())
	{
		...
		while (it != m_EventList.end())
		{
			...
			{		
				...
				else  //说明在事件发出TICK信息还没得到响应，500个周期（至少大约10秒）后检查事件地址是否依然有效
				{     //如果有效继续等待，理论上对方一定会处理
...
void CBrain::OutputLog(uint32 Flag,const TCHAR* s){
	if (m_LogFlag & Flag)
	{
		...
		else
		{
			tstring time = GetTimer()->GetHMSM(TimeStamp);
			CLogicDialog* SysDialog = m_BrainData.GetDialog(SYSTEM_SOURCE,DEFAULT_DIALOG);
			SysDialog->RuntimeOutput(0,_T("%s %s"),time.c_str(),s);
		}
	}
}
...
bool CBrain::Login(int64 SourceID,tstring UserName, tstring CrypStr)
{
	...
	m_BrainData.RegisterOrgan(UserName,CrypStr,SourceID,DIALOG_SYSTEM);
...
bool CBrain::CreateSysDialog(int64 SourceID,tstring SourceName,bool bEdit){
	...
	ePipeline ClientAddress(SourceID);
...
void CBrain::StartSysTask(SysTaskType SysTask,int64 SourceID,ePipeline& Param,tstring TaskName,int64 ClientEventID,bool bOnce){
	...
	if(!ChildDialog){
		...
		tstring s = Format1024(_T("Start System task '%s' "),TaskName.c_str());
		OutSysInfo(s.c_str());
	}
