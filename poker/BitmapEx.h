...
public:
	...
	void SaveTest(LPTSTR lpszBitmapFile);
	void Save(LPBYTE lpBitmapData);
	long Save(LPBYTE lpBitmapData,long size);//#ZHB添加,给定buf,成功返回实际长度
	long GetSaveSize(); //#ZHB添加，返回存储所需字节数

	long AutoThreshold();//#ZHB添加,先变灰度，自动根据灰度组成选择合适的阀值二值化
	...
	//#zhb
	void DrawTextA(HDC hDC,long dstX, long dstY, LPSTR lpszText, _PIXEL textColor, long textAlpha, LPTSTR lpszFontName, long fontSize, BOOL bBold=FALSE, BOOL bItalic=FALSE);
