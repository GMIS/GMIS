...
void CBitmapEx::SaveTest(LPTSTR lpszBitmapFile)
{
	int ImageSize   = sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER);
	ImageSize +=GetPaletteEntries()*sizeof(RGBQUAD);
	ImageSize +=+GetSize();
	...
	if (file != NULL)
	{
		fwrite(ImageData, ImageSize, sizeof(BYTE), file);
...
long  CBitmapEx::GetSaveSize(){

		// Convert to 24bpp bitmap
		_ConvertTo24Bpp();
		
		int size = sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER)+m_dwSize*sizeof(BYTE);
        if (m_bih.biBitCount == 8){
			size += m_iPaletteEntries*sizeof(RGBQUAD);
		}
		_ConvertTo32Bpp();
...
long CBitmapEx::Save(LPBYTE lpBitmapData,long size)//#ZHB添加,成功返回长度
{
	int len = 0;
	// Convert to 24bpp bitmap
	_ConvertTo24Bpp();

	len = sizeof(BITMAPFILEHEADER)+sizeof(BITMAPINFOHEADER)+m_dwSize*sizeof(BYTE);
	if (m_bih.biBitCount == 8){
		len += m_iPaletteEntries*sizeof(RGBQUAD);
	}
...
long CBitmapEx::AutoThreshold()
{
	Grayscale();
	//灰度分布统计
    long Gray[256]={0};
	
	long iWidth = m_bih.biWidth, iHeight =m_bih.biHeight;
	long x = 0,y = 0,i = 0,t = 0;
...
void CBitmapEx::DrawTextA(HDC hDC,long dstX, long dstY, LPSTR lpszText, _PIXEL textColor, long textAlpha, LPTSTR lpszFontName, long fontSize, BOOL bBold, BOOL bItalic)
{
	// Check for valid bitmap
	if (IsValid() && (lpszText != NULL))
	{
